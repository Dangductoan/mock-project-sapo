...
function ManageAccountant(props) {
  ...
  useEffect(() => {
    AccountantService.getAccountant().then((res) => {
      const data = res.data;
      console.log(data);
    ...
    <>
      <div className="manageAccoutant ml-230">
        <div className="manageAccoutant-header">
          <h2 className="manageAccoutant-title">Danh sách nhân viên</h2>
        ...
        <div className="manageAccoutant-content">
          <div className="manageAccoutant-table">
            ...
            {accountants.map((account) => {
              const { id, name, username, phoneNumber, address, createdAt } =
                account;
              const data = account.createdAt.toString().slice(0, 10);
              return (
                <ListAccountant
                  key={id}
                  index={id}
